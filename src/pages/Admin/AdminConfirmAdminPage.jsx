...
export default function AdminConfirmAdminPage() {
  ...
  const pendingAdmins = useMemo(() => {
    return admins.filter((a) => (a.adminApproval?.status || "pending") === "pending");
  }, [admins]);

  const fetchRequests = async () => {
    setLoading(true);
    try {
      const res = await usersApi.listAdminRequests("pending");
      setAdmins(res.data?.admins || []);
    } catch (err) {
      setToast(err.response?.data?.message || "Failed to load admin requests");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openApprove = (admin) => {
    setSelected(admin);
    setApproveOpen(true);
  };

  const confirmApprove = async () => {
    const adminId = selected?._id || selected?.id;
    if (!adminId) return;

    try {
      await usersApi.approveAdmin(adminId);
      setToast("Admin approved ✅");
      setAdmins((prev) => prev.filter((a) => (a._id || a.id) !== adminId));
    } catch (err) {
      setToast(err.response?.data?.message || "Failed to approve admin");
    } finally {
  ...
  const confirmReject = async () => {
    ...
    try {
      await usersApi.rejectAdmin(adminId, rejectNote);
      setToast("Admin rejected ❌");
      setAdmins((prev) => prev.filter((a) => (a._id || a.id) !== adminId));
    } catch (err) {
      setToast(err.response?.data?.message || "Failed to reject admin");
    } finally {
      setRejectOpen(false);
      setSelected(null);
      setRejectNote("");
  ...
  return (
    <div className="min-h-screen bg-[#F7FBF2] flex">
        ...
        <div className="p-6">
          ...
          <div className="bg-white rounded-xl border border-[#E6F4EC] shadow-sm overflow-hidden">
            ...
            {loading ? (
              ...
              <div className="p-6 text-sm text-[#5B7065]">No pending admin requests.</div>
            ) : (
                    ...
                    {pendingAdmins.map((a) => {
                      ...
                      return (
                        <tr key={id} className="border-t border-[#E6F4EC]">
                          <td className="px-5 py-3 text-[#124734] font-medium">{a.fullName || "-"}</td>
                          <td className="px-5 py-3 text-[#5B7065]">{a.email || "-"}</td>
                          <td className="px-5 py-3 text-[#5B7065]">{a.phone || "-"}</td>
                          <td className="px-5 py-3 text-[#5B7065]">{a.city || "-"}</td>
                          <td className="px-5 py-3 text-[#5B7065]">{a.state || "-"}</td>
      ...
      <ConfirmDialog
        open={approveOpen}
        title="Approve Admin?"
        message={`Are you sure you want to approve ${selected?.fullName || "this admin"}? They will be able to login.`}
        onCancel={() => {
          setApproveOpen(false);
          setSelected(null);
        }}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[5000]">
          <div className="bg-white w-[520px] rounded-xl shadow-xl p-6 border border-[#E6F4EC]">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-lg font-semibold text-[#124734]">Reject Admin?</h2>
              <button
                onClick={() => {
                  setRejectOpen(false);
                  setSelected(null);
